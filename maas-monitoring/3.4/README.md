...
Adds a **Token metrics** tab on OpenShift AI **Observe & monitor → Dashboard (Tech Preview)**. It sorts after **Usage**. Leave the product Usage dashboard (`dashboard-3-maas-usage-admin`) in place.
...
The 3.5 COO YAML is in [`../3.5/`](../3.5/). Do not apply 3.4 and 3.5 together. Do not apply this folder on a 3.5 cluster.
...
Complete these steps in order. Apply the dashboard only in step 4, after Limitador is already scraped. If a Limitador scrape already exists, do not add another ServiceMonitor or PodMonitor.
...
`limitador-servicemonitor.yaml` is an optional fallback scrape. It is not listed in `kustomization.yaml`.
...
If Usage uses another name, change every datasource `name` in `dashboard-4-maas-token-metrics-admin.yaml` to match before you apply.
...
# In the Prometheus / Thanos used by the datasource:
#   authorized_hits
```

If `podmonitor/kuadrant-limitador-monitor` exists in `kuadrant-system`, or the series already exists in User Workload Prometheus, **stop**. A second scrape duplicates series.
...
This dashboard queries untyped **`authorized_hits`** (same as Usage on RHOAI 3.4), labels `user`, `subscription`, `model`. Grouping is by **subscription** (`MaaSSubscription` name).

Stay on this folder on a 3.4 cluster even if you also see `authorized_hits_total`. The [3.5](../3.5/) folder is for OpenShift AI 3.5 / COO, not for renaming the metric.
...
1. Scrape: no `authorized_hits` in the datasource Prometheus. Re-run step 3.
